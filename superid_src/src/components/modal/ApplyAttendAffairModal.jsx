...
/* 申请加入事务模态框 */
const ApplyAttendAffairModal = React.createClass({
  getDefaultProps() {
    return {
      menkor: false,
    }
      ...
      if (errors){
        ...
        fetch(config.api.affair.join.apply(affair.get('id'), role.get('roleId'), form.getFieldValue('reason')), {
          ...
          if (json.code == 0 || json.code == 103) {
            ...
            form.resetFields()
            message.success('已发送加入事务申请给事务管理员，请耐心等待')
...
export default connect(mapStateToProps, mapDispatchToProps)(Form.create()(ApplyAttendAffairModal))
