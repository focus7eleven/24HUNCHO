...
export function eventLevels(rowSegments, limit = Infinity) {
    ...
    for (i = 0; i < rowSegments.length; i++) {
        ...
        for (j = 0; j < levels.length; j++) {
            if (levels[j].left == seg.left) {
                levels[j].events.push(seg.event);
                flag = true;
            }
        }
        if (!flag) {
            seg.events = [seg.event];
            levels.push(seg);
        }
    }
    levels.sort((a, b) => a.left - b.left); //eslint-disable-line
...
export function myEventLevels1(rowSegments, limit = Infinity) {
    let levels = [],
        extra = [];
    if (rowSegments[0]) {
        levels[0] = rowSegments[0];
    }
