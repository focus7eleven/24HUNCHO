...
const downloadFile = (url, affair) => {
  const extraInfo = `response-content-disposition=${encodeURIComponent(`attachment;filename="${url.split('/').slice(-1)[0]}"`)}`

  return fetch(config.api.file.token.simple_download(url, extraInfo),{
    method: 'POST',
    ...
    if (json.code === 0) {
      ...
      link.href = `${data.host}/${data.path}?OSSAccessKeyId=${data.accessId}&Expires=${data.expireTime}&Signature=${encodeURIComponent(data.signature)}&${extraInfo}`
      link.click()
...
//使用带有进度的上传, 只返回path，不返回host
const uploadNeedDownloadFileToOSSWithProgress = (file, fileId, host, path, accessId, expires, signature, headers={}, onProgressChange, onabort) => {
  let xhr = new XMLHttpRequest();
  xhr.open('PUT', `${host}/${encodeURIComponent(path)}?OSSAccessKeyId=${accessId}&Expires=${expires}&Signature=${encodeURIComponent(signature)}`, true);
  onabort && onabort(xhr);
...
export default {
  ...
  uploadChatFile(file, affairId, roleId, onProgressChange = null, onabort = null) {
    return fetch(config.api.file.token.chat(), {
      ...
      if(json.code === 0) {
        const data = json.data;
        return uploadNeedDownloadFileToOSSWithProgress(file, 0, data.host, data.path, data.accessId, data.expireTime, data.signature, {}, onProgressChange, onabort);
      }
      else if(json.code === 403) {
        throw new Error("权限不足");
      }
