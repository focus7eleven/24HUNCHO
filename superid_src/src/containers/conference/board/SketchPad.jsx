...
export default class SketchPad extends React.Component {
  ...
  componentWillReceiveProps({ tool, items, remoteType }) {
    ...
    // if (suspensionMode !== this.props.suspensionMode) {
    //   this.renderItems(items)
    // }
  ...
  onMouseDown(e) {
    const { operation, writePermission } = this.props    

    if (!writePermission) {
      message.warning('白板已锁，您无权限进行操作！')
      return
    }
    switch (operation) {
  ...
  onrenderTextArea(e) {
    ...
    textarea.placeholder = '按 Enter 完成输入'
    setTimeout(() => {
      textarea.focus()
    }, 0)
  }
  ...
  render() {
    const { width, height, scale, suspensionMode } = this.props
    const { isTexting } = this.state

    let bgStyle = { width: (width * scale) + 'px', height: (height * scale) + 'px' }
    
    return (
      <div className={styles.sketchPad}>
        <div ref={(b) => this.canvasBg = b} className={styles.canvasBackground} style={bgStyle}>
          <svg
            ref={(d) => this.rect = d}
        ...
        <canvas
          ...
          className={classNames(styles.canvas, suspensionMode ? 'suspensionMode' : '')}
