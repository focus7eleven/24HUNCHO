...
import { Dropdown, Menu, Slider, Popconfirm, message } from 'antd'
...
class WhiteBoard extends React.Component {
  ...
  handleUndo() {
    const { isHost, isLock } = this.props
    if (!isHost && isLock) {
      message.warning('白板已锁，您无权限进行此操作！')
      return
    }
  ...
  changeWritePermission() {
    ...
    this.props.changeWritePermission(!isLock)
  }
      ...
      <Menu className={styles.customMenu}>
        ...
        <Menu.Item>
          <div className={styles.menuItem}>
            <Popconfirm placement="bottom" title="确认将目前白板画面清空" onConfirm={() => this.handleClearAll()} okText="是" cancelText="否">
  ...
  renderScaleMenu() {
    ...
    return (
      <div className={styles.scaleMenu}>
        ...
        <div className={styles.text}>150%</div>
      </div>
    )
  }

  render() {
    const { items, remoteType, containerWidth, containerHeight, isHost, isLock, suspensionMode } = this.props
    const {
      color,
      colorVisible,
      operation,
    ...
    const writePermission = isHost || !isLock

    return (
      <div className={styles.whiteBoard}>
        <div className={classNames(styles.editorBar, suspensionMode ? 'hide' : '')}>
          ...
          <div className={styles.right}>
            {isLock ? <EditorBtn type="locked" text="已锁" onClick={this.changeWritePermission.bind(this)}/> : <EditorBtn type="unlock" text="未锁" onClick={this.changeWritePermission.bind(this)}/>}
          </div>
